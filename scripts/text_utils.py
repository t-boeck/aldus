...
def split_paragraphs(text):
    # Split on 2 or more newlines, allowing for whitespace in between
...
def bold_first_word(sentence: str, language: str = "chinese") -> str:
    ...
    for i, token in enumerate(tokens):
        ...
        if language == "chinese":
            ...
            # For Spanish/French/English, just bold the whole token (first word)
            bolded_token = leading + r"\textbf{" + latex_escape(the_rest) + "}"
...
def process_paragraph(paragraph, language="chinese", line_stretch=1.3):
    sentences = split_into_sentences(paragraph)
    bolded_sentences = [bold_first_word(s, language) for s in sentences]
